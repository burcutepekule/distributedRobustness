...
if(length(numOfGates)==1)
    ...
    numOfGatesVec      = numOfGates;
    keepCircuits       = {};
    keepStructure      = {};
    circuitIdx         = 1;
    strAll             = [ ];
    for m=1:length(numOfGatesVec)
        numOfGates = numOfGatesVec(m);
        ...
        while(circuitFound==0)
            ...
            if(allOutConnected==1 && allNodesUsed==1 && noBackwardConn==1)
                ...
                [~,idx] = unique(cellfun(@num2str,strAll,'uni',0));
                if(circuitIdx<=length(idx)) %if the new one is unique, keep
                    disp(sprintf('Found circuit number %d, size : %d',circuitIdx,numOfGatesBetween))
                    circuitIdx   = circuitIdx + 1;
                    circuitFound = 1;
                else% remove
                    keepCircuits{circuitIdx}    = [];
