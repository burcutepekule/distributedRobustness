function [degeneracy,degeneracy2,degeneracyUB,redundancy,complexity,circuitSize] = calculateDegeneracy(keepOutput,keepAllOutput,numOfInputs,numOfOutputs,fittestStructure)
...
for k=1:length(middleGates)-1
    ...
    for i=1:size(gates2use,1)
        ...
        gatesSubTempVec = [gatesSubTempVec gatesSubTemp];
...
%%%%%%%%%%%%%%%%%% FOR REDUNDANCY %%%%%%%%%%%%%%%%%%
middleGates   = setdiff(allGates,inputGates); %TONNI PAPER
disp(['Calculating redundancy...'])
gates2use     = nchoosek(middleGates,1);
IallVec       = [];
IsubVec       = [];
gatesSubTempVec=[];
for i=1:size(gates2use,1)
