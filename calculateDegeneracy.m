function [degeneracy,degeneracyUB,redundancy,complexity,circuitSize] = calculateDegeneracy(keepOutput,keepAllOutput,numOfInputs,numOfOutputs,fittestStructure)
...
%         round([Isub;IsubHat;Iall;(Isub+IsubHat-Iall)],3)%Table 4
    ...
    IallVecMean       = [IallVecMean mean(IallVec)]; % <I(X,O)> (this is same anyway for each subcircuit)
    IsubVecMean       = [IsubVecMean mean(IsubVec)]; % <I(X^k,O)>
...
redundancy   = sum(IsubVecMean)-mean(IallVecMean); % sum(<I(X^k,O)>)-I(X,O)
