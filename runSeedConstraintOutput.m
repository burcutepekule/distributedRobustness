function [] = runSeedConstraintOutput(seed,outputMat,numOfInputs,numOfOutputs,numOfGates,numOfRuns,numOfCandidateSolutions)
...
save(['/net/cephfs/data/btepek/DISTRIBUTEDROBUSTNESS/BEFORE_TOL_FITTEST_CIRCUIT_CONSTRAINT_SEED_' num2str(seed) '_' num2str(sim) '.mat'])
...
while(maxFitness<1)
    ...
    % constraint the size of the circuit, meaning that if gate addition
    % happens, gate removal also has to happen - this will be the
    % mutationIndex 4. So sample either from index 1 or 4 with equal probs.
    
    mutationIndexVec = datasample([1 4],numOfCandidateSolutions-1,'Weights',[0.5 0.5],'Replace',true);
...
save(['/net/cephfs/data/btepek/DISTRIBUTEDROBUSTNESS/BEFORE_TOL_ALL_CONSTRAINT_SEED_' num2str(seed) '.mat'])
...
%     mutationIndexVec = datasample(1:3,numOfCandidateSolutions-1,'Weights',[0.8 0.3 0.6],'Replace',true);
    ...
    save(['/net/cephfs/data/btepek/DISTRIBUTEDROBUSTNESS/AFTER_TOL_FITTEST_CIRCUIT_CONSTRAINT_SEED_' num2str(seed) '_' num2str(sim) '.mat'])
    sim = sim + 1;
end
...
save(['/net/cephfs/data/btepek/DISTRIBUTEDROBUSTNESS/AFTER_TOL_ALL_CONSTRAINT_SEED_' num2str(seed) '.mat'])
