function [] = runSeedSizeOutput(seed,outputMat,numOfInputs,numOfOutputs,numOfGates,numOfCandidateSolutions,mult,preDefinedSize,L,mutProb)
...
save(['/net/cephfs/data/btepek/DISTRIBUTEDROBUSTNESS/BEFORE_TOL_FITTEST_CIRCUIT_SIZE_SEED_' num2str(seed) '_' num2str(sim) '.mat'])
...
while(sim<1e4)  % lets run as long as possible
    ...
    if(mod(sim,100)==0) %save every
        if(mutProb==0)
            ...
            save(['/net/cephfs/data/btepek/DISTRIBUTEDROBUSTNESS/BEFORE_TOL_FITTEST_CIRCUIT_SIZE_MUT_SEED_' num2str(seed) '_' num2str(sim) '.mat'])
        end
    end
    sim = sim + 1;
end
save(['/net/cephfs/data/btepek/DISTRIBUTEDROBUSTNESS/BEFORE_TOL_ALL_SIZE_SEED_' num2str(seed) '.mat'])
