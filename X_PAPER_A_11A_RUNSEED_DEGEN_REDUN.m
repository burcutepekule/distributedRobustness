...
ind_min = i(ind); 
ind_max = i(ind+1); 
plot10panel_a(textCircuits,keepStructure,IsubOneVals,IallFinalVals,degeneracyVals,redundancyVals,Ifinal,IsubMatKeepVals,ind_min,ind_max,0,1)
...
ind_min = i(ind); 
ind_max = i(ind+1); 
plot10panel_a(textCircuits,keepStructure,IsubOneVals,IallFinalVals,degeneracyVals,redundancyVals,Ifinal,IsubMatKeepVals,ind_max,ind_min,0,1)
%REDUNDANCY VALUES CAN BE OBTAINED BY THE SUM OF FIRST ROW (k=1) - I(X,O)
[nansum(IsubMatKeepVals(1,:,ind_min))-IallFinalVals(ind_min) nansum(IsubMatKeepVals(1,:,ind_max))-IallFinalVals(ind_max)]
%DEGENERACY VALUES CAN BE OBTAINED BY THE SUM OF THE COLUMN MEANS
[0.5*sum(nanmean(Ifinal(:,:,ind_min),2)) 0.5*sum(nanmean(Ifinal(:,:,ind_max),2))]
...
plot10panel_a(textCircuits,keepStructure,IsubOneVals,IallFinalVals,degeneracyVals,redundancyVals,Ifinal,IsubMatKeepVals,ind_zero,0,1)
%REDUNDANCY VALUES CAN BE OBTAINED BY THE SUM OF FIRST ROW (k=1) - I(X,O)
[nansum(IsubMatKeepVals(1,:,ind_min))-IallFinalVals(ind_min) nansum(IsubMatKeepVals(1,:,ind_max))-IallFinalVals(ind_max)]
%DEGENERACY VALUES CAN BE OBTAINED BY THE SUM OF THE COLUMN MEANS
[0.5*sum(nanmean(Ifinal(:,:,ind_min),2)) 0.5*sum(nanmean(Ifinal(:,:,ind_max),2))]
