function [normNew,connectionMat_text] = drawCircuit_calc_dist(structureTemp,textCircuitsTemp)
...
for l=1:size(structureTemp,1)
    ...
    if(layerTemp==0) %input layer
        for i=1:gatesTemp
            outletPointsInpTemp = drawINPUT(xPlotTemp,yPlotTemp(i),0);
            outletPointsInp     = [outletPointsInp; repmat(layerTemp,1,1) repmat(i,1,1) outletPointsInpTemp];
        end
    else %mid layers
        for i=1:gatesTemp
            [inletPointsTemp,outletPointsTemp] = drawNAND(xPlotTemp,yPlotTemp(i),0);
...
for i=1:numOfOutputs
     inletPointsOutTemp = drawOUTPUT(xPlot(end),yPlotTemp(i),0);
...
normNew = 0;
connectionMat_begin=[connectionMat_x(:,1) connectionMat_y(:,1)];
connectionMat_end  =[connectionMat_x(:,2) connectionMat_y(:,2)];

for k=1:size(connectionMat_begin,1)
    normNew = normNew + norm(connectionMat_begin(k,:)-connectionMat_end(k,:));
end
