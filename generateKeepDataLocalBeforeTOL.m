function [] = generateKeepDataLocalBeforeTOL(seed)
if(isfile(['./cluster/BEFORE_TOL_ALL_SEED_' num2str(seed) '.mat']))
    ...
    load(['./cluster/BEFORE_TOL_ALL_SEED_' num2str(seed) '.mat']);
...
%     [degeneracy,degeneracy2,degeneracyUB,redundancy,complexity,circuitSize] = calculateDegeneracy(keepOutput,keepAllOutput,fittestStructure);
    [degeneracy,degeneracy2,degeneracyUB,redundancy,complexity,circuitSize] = calculateDegeneracy2(keepOutput,keepAllOutput,fittestStructure);
...
%     load(['./cluster/BEFORE_TOL_FITTEST_CIRCUIT_SEED_' num2str(seed) '_1.mat']);
%     %     faultTolerance               = faultTolerance(end,fittestCircuitIdx);
%     faultTolerance               = nan;
%     fitness                      = fitness(end,fittestCircuitIdx);
%     keepDataInitial               = [];
...
%     [degeneracy,degeneracy2,degeneracyUB,redundancy,complexity,circuitSize] = calculateDegeneracy(keepOutput,keepAllOutput,fittestStructure);
%     
%     keepDataInitial = [keepDataInitial; size(fitness,1) circuitSize fitness ...
%         faultTolerance degeneracy degeneracy2 degeneracyUB ...
%         redundancy complexity];
    
    save(['./cluster/RDC_ALL_SEED_' num2str(seed) '_BTOL.mat']) %REDUNDANCY, DEGENERACY, COMPLEXITY
else
